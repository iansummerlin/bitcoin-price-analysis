...
These classes translate a prediction stream into discrete ENTRY / EXIT / HOLD
decisions.  They exist solely to support the evaluation harness — they are
not trading strategies in the production sense.
"""
...
class SignalRule:
    """Base interface for translating predictions into discrete signals."""

    def on_candle(self, row, simulator) -> Signal:
        raise NotImplementedError

    def get_position_size_usd(self, row, simulator) -> float:
        """How much USD to allocate for a simulated entry. Override for custom sizing."""
        return min(100.0, simulator.cash)


class ThresholdRule(SignalRule):
    """
    Simple threshold rule used as a baseline signal consumer.
    Signals ENTRY if predicted price > current * buy_threshold.
    Signals EXIT if predicted price < current * sell_threshold.
    """
...
class ATRThresholdRule(SignalRule):
    """
    Dynamic threshold rule using ATR for volatility-adaptive thresholds.
    ...
    def get_position_size_usd(self, row, simulator) -> float:
        # Inverse volatility sizing: smaller when volatile
        atr = row.get('atr_24', 0)
        current = row['close']
        if atr == 0 or current == 0:
            return min(100.0, simulator.cash)
        volatility_ratio = atr / current
        # Base size of 5% of simulated value, scaled down by volatility
        base_size = simulator.get_portfolio_value(current) * 0.05
        size = base_size / (1 + volatility_ratio * 100)
        return min(size, simulator.cash)


class MultiFactorRule(SignalRule):
    """
    Multi-factor confirmation rule for evaluation.
    Requires multiple conditions to align before signalling:
    ...
    def _check_drawdown_breaker(self, simulator) -> bool:
        """Returns True if drawdown circuit breaker is triggered."""
        if len(simulator.equity_curve) < 2:
            return False
        peak = max(simulator.equity_curve)
        current = simulator.equity_curve[-1]
        if peak == 0:
            return False
    ...
    def on_candle(self, row, simulator) -> Signal:
        # Circuit breaker: stop signalling entries if drawdown too large
        if self._check_drawdown_breaker(simulator):
            # Still allow exits to reduce exposure
        ...
        # --- Stop-loss check ---
        if simulator.has_position() and simulator._open_position:
            entry = simulator._open_position['entry_price']
    ...
    def get_position_size_usd(self, row, simulator) -> float:
        """Risk-based position sizing for evaluation."""
        current = row['close']
        atr = row.get('atr_24', 0)

        portfolio_value = simulator.get_portfolio_value(current)
        ...
        # Cap at available cash and 20% of simulated value
        max_size = portfolio_value * 0.20
        return min(size, max_size, simulator.cash)


# -- Backward-compatible aliases so existing callers keep working during
#    migration.  Import from this module using the new names going forward.

Portfolio = None  # sentinel — see cost_model.py for the real class
Strategy = SignalRule
ThresholdStrategy = ThresholdRule
ATRThresholdStrategy = ATRThresholdRule
MultiFactorStrategy = MultiFactorRule
